 ...
 * ----------------------------------------- USER SPACE ----------------------------------------- */
	...
	#define STATIC_EXECNAME		"./mlbtcc.a"
	...
	#define BUILD_LEVELDB_CMD	"mkdir -p build && cd build && \
...
{
	if (!FileExists("src/extern/leveldb-1.22/build/libleveldb.a") || HasArg(ac, av, "force"))
	{
		Command *cmd = CreateCommand();
		AppendToCommand(cmd, "cd src/extern/leveldb-1.22/ &&", BUILD_LEVELDB_CMD);
		ExecuteCommand(cmd);
	}
}

void	BuildStatic(int ac, char **av)
{
	char cflags[256];
	char ldflags[256];
	...
	if (HasArg(ac, av, "debug"))
	{
		...
		strcat(ldflags,	" -fsanitize=address,undefined");
		strcat(cflags,	" -g3");
	}

	CompileFiles("src/extern/sha256/*.c", "build", CC, cflags);
	CompileFiles("src/*.c", "build", CC, cflags);
	ExecuteCommand(CreateCommand("ar rcs", STATIC_EXECNAME, "build/*.o"));
}

void	BuildDynamic(int ac, char **av)
{
	char cflags[256];
	char ldflags[256];
	strcpy(cflags, CFLAGS);
	...
	CompileFiles("src/extern/sha256/*.c", "build", CC, cflags);
	CompileFiles("src/*.c", "build", CC, cflags);
	LinkFiles("build/*.o", DYNAMIC_EXECNAME, LD" -dynamiclib" , ldflags);
}
...
int main(int argc, char **argv)
{
	...
	CreateRule("build-tests",		BuildTests);
