...
typedef struct OpenFileCache
{
	...
	U32			lastAccessTime;	// Last access time to implement LRU
} OpenFileCache;
...
{
	U8 	version;			// 4 bytes - Block Version
	U8 	prevBlockHash[32];	// 32 bytes - Previous Block Hash
	U8 	merkleRoot[32];		// 32 bytes - Merkle Root of all the transactions
	U32 	time;				// 4 bytes - UNIX timestamp
	U32 	bits;				// 4 bytes - Target
	U32 	nonce;				// 4 bytes - Nonce
...
{
	U8		txid[32];			// 32 bytes - Natural byte order
	U32	vout;				// 4 bytes - Little-endian
	uint16_t	scriptSigSize;		// Variable size - Compact size
	U8		*scriptSig;			// Variable size - Script (unlocking code)
	U32	sequence;			// 4 bytes - Little-endian
} Input;

// SEE: https://github.com/bitcoin/bitcoin/blob/60b816439eb4bd837778d424628cd3978e0856d9/src/primitives/transaction.h#L149
typedef struct Output
{
	U64	amount;				// 8 bytes - Little-endian (value in satoshis)
	uint16_t	scriptPubKeySize;	// 2 bytes - Compact size
	U8		*scriptPubKey;		// Variable size - Script (locking code)
} Output;

typedef struct StackItem
{
	uint16_t size;           // Variable size - Compact size
	U8 *item;           // Variable size - Bytes (data to push onto stack)
} StackItem;
...
{
	U32			version;		// 4 bytes - little-endian
	U8			marker;			// 1 byte - segwit marker
	U8			flag;			// 1 byte - segwit flag
	...
	U32			locktime;		// 4 bytes - little-endian
	uint16_t	size;
	U8			txid[32];
	U8			wtxid[32];
...
{
	U8 blockHash[32];				// 32 bytes - Block Hash -- Manually reversed it
	BlockHeader blockHeader;		// 80 bytes - Raw Block Header
	U64 version;					// 8 bytes - Client version
	U64 height;						// 8 bytes - Block Height
	U64 txCount;					// 8 bytes - number of transactions in the block
	BlockStatus validationStatus;	// 4 bytes - validation status (enum)
	U64 blockFile;					// 8 bytes - block file number
	U64 blockOffset;				// 8 bytes - where the block is located in blk.dat
	U64 undoFile;					// 8 bytes - undo file number
...
typedef struct FileInformationRecord
{
	...
	U64 blockCount;					// 8 bytes - The number of blocks stored in the block file with that number
	U64 dataFileSize;				// 8 bytes - The size of the blk.dat file with that number
	U64 revFileSize;				// 8 bytes - The size of the rev.dat file with that number
	U64 lowestHeight;				// 8 bytes - The lowest height of blocks stored in the block file with that number
	U64 highestHeight;				// 8 bytes - The highest height of blocks stored in the block file with that number
	U64 lowestTimestamp;			// 8 bytes - The lowest timestamp of blocks stored in the block file with that number
	U64 highestTimestamp;			// 8 bytes - The highest timestamp of blocks stored in the block file with that number
...
typedef struct BlockStats
{
	...
	U64 avgFee;			// WARN: Probably not easily computable
	U64 avgFeeRate;		// WARN: Probably not easily computable
	U64 avgTxSize;
	U8 blockHash[32];
	U64 height;
	U64 ins;
	U64 maxFee;			// WARN: Probably not easily computable
	U64 maxTxSize;
	U64 minFee;			// WARN: Probably not easily computable
	U64 minFeeRate;		// WARN: Probably not easily computable
	U64 outs;
	U64 subsidy;
	U64 swTotalSize;
	U64 swTotalWeight;
	...
	U64 totalSize;
	U64 totalWeight;
	U64 totalFee;			// WARN: Probably not easily computable
	U64 txs;
	U64 utxoIncrease;
} BlockStats;
